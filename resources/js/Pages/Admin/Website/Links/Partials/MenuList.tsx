...
import MenuTable from "@/Interfaces/DataTable/MenuTableProps";
import {Block} from "@/models/block/Block";
import MenuService from "@/Services/MenuService/MenuService";

const headCells: readonly HeadCell<MenuTable>[] = [
    {
        id: 'id',
        numeric: false,
...
const MenuList: React.FC<{category: string, menus: Menu []}> = ({category, menus}) => {
    const tableService = Container.get(TableService);
    const menuService = Container.get(MenuService);
    const commonService = Container.get(CommonService);

    const [selectedMenus, setSelectedMenus] = useState<Menu []>(menus);
    const [selectedMenu, setSelectedMenu] = useState<Menu>(new Menu({}))
    ...
    const deleteMenu = (id: number) => {
        ...
        menuService.deleteMenu(id)
            ...
            .catch(error => {
                setSnackbar(snackbarState =>
                    ({ ...snackbarState, open: true, message: 'Error Happened while deleting menu!', severity: "error" })
                );
            })
    }
    // =========================================================================================
                        ...
                        {visibleRows.map((row, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <TableCell align="right">
                                        <Link href={`/admin/website/menu/${row.id}`}>
            ...
            >
                <Box className={styles.modalStyle} sx={{
                    ...
                    <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                        Are you sure that you want to delete {menuService.getMenuName(selectedMenu)}
                    </Typography>
                    <Stack
                        direction="row"
                    ...
                    >
                        ...
                        <CustomButton task="delete" text="" onClick={() => deleteMenu(selectedMenu.id)}></CustomButton>
                    </Stack>
                </Box>
            </Modal>
            <CustomSnackbar
