...
const CustomLinks: React.FC<{socialLinks: MenuLink [], contactLinks: MenuLink []}> = ({socialLinks, contactLinks}) => {
    ...
    // React.useEffect(()=> {}, [socialLinks, contactLinks]);
    // const [github, setGithub] = React.useState<MenuLink>();
    // React.useEffect(() => {
    //     // if (socialLinks.length > 0)
    //     setGithub(socialLinks.find(item => item.name.toLowerCase() === "github"));
    //     console.log(socialLinks.find(item => item.name.toLowerCase() === "github"));
    // }, [socialLinks]);
